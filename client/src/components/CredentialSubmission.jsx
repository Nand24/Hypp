...
import TransferGuideChecklist from "./TransferGuideChecklist";
...
const CredentialSubmission = ({ onClose, listing }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-100 flex items-center justify-center sm:p-4">
            <div className="bg-white sm:rounded-lg shadow-2xl w-full max-w-lg h-screen sm:h-[560px] flex flex-col">
                {/* Header */}
                ...
                {/* Form */}
                <form onSubmit={handleSubmission} className="flex flex-col items-start gap-4 p-4 overflow-y-scroll flex-1">
                    ...
                    <div className="flex items-center gap-2">
                        <input value={newField} onChange={(e) => setNewField(e.target.value)} type="text" placeholder="field name ..." className="outline-none border-b border-gray-200 text-sm" />
                    ...
                    <div className="w-full my-2">
                        <TransferGuideChecklist interactive={false} initialCompleted={[1, 2]} />
                    </div>

                    {/* Submit Button */}
                    <button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 mt-2 rounded-md font-medium cursor-pointer w-full">
                        Submit Credentials to Escrow
                    </button>
                </form>
            </div>
        </div>
